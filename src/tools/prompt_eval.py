...
class CustomModel(nn.Module, PyTorchModelHubMixin):
    def __init__(self, target_sizes, task_type_map, weights_map, divisor_map):
        ...
        self.heads = [
            MulticlassHead(self.backbone.config.hidden_size, z)
            for z in self.target_sizes
        ]
    ...
    def compute_results(self, preds, target, decimal=4):
        if target == "task_type":
            ...
            weighted_sum = np.sum(preds_np * weights, axis=1)
            scores = weighted_sum / self.divisor_map[target]

            scores = [round(float(value), decimal) for value in scores]
...
class PromptComplexityPredictor:
    def __init__(self):
    ...
    def __call__(self, prompts):
        """Single method to handle both single prompt and batch of prompts"""
